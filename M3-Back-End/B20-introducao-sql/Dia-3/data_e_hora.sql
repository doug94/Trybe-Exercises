...
SELECT COUNT(*) FROM rental
WHERE DATE(rental_date) BETWEEN '2005-07-01' AND '2005-08-22';

-- 3 Usando a tabela rental, extraia data, ano, mês, dia, hora, minuto e segundo dos registros com rental_id = 10330.
SELECT DATE(rental_date) AS Data, YEAR(rental_date) AS Ano, MONTH(rental_date) AS Mes, DAY(rental_date) AS Dia,
HOUR(rental_date) AS Hora, MINUTE(rental_date) AS Minuto, SECOND(rental_date) AS Segundo
FROM rental
...
AND TIME(payment_date) >= '22:00:00';
